...
trait IntegerType extends TruffleGen with Types {
  ...
  case class IntMinus(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) - y.execute(frame)
    }
  }
  case class IntTimes(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) * y.execute(frame)
    }
  }
  case class IntMod(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) % y.execute(frame)
    }
  }
  case class IntDiv(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) / y.execute(frame)
    }
  }
  case class IntEqual(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Boolean] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) == y.execute(frame)
    }
  }
  case class IntNotEqual(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Boolean] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) != y.execute(frame)
    }
  }
  case class IntLess(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Boolean] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame) < y.execute(frame)
    }
  }

  case class IntToInt(@(Child @field) x: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
      x.execute(frame)
    }
  }

  case class IntBitAnd(@(Child @field) x: ExpNode[Int], @(Child @field) y: ExpNode[Int]) extends DefNode[Int] {
    def execute(frame: VirtualFrame) = {
  ...
  case class IntToString(@(Child @field) x: ExpNode[Int]) extends DefNode[String] {
    def execute(frame: VirtualFrame) = {
...
//
//  override def int_plus(x: ExpNode[Int], y: ExpNode[Int]) = (x, y) match {
