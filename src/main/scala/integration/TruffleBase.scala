...
trait TruffleBase extends Base {
  ...
  def reify[T: Typ](d: => Exp[T]): BlockNode[T] = {
    val save = localDefs
  ...
  case class BlockNode[T: Typ](@(Children @field) stms: Array[Stm], res: Exp[T]) extends Node {
  ...
  class LMSNestedRootNode[@specialized T](@(Child @field) val block: BlockNode[T]) extends RootNode {
